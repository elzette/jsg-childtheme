	...
	*/
	...
	if ( have_posts() ) : 
		...
		while ( have_posts() ) : the_post();
			...
			<figure class="featured-image" style="background-image: url('<?php echo $background[0]; ?>'); background-position: center;">
			<?php echo '</figure>' . get_the_post_navigation() . '<div class="wrap"><article class="post-' . $post->ID . ' entry" itemscope="" itemtype="http://schema.org/CreativeWork">';
			...
			if ($architect || $egineer) {
				...
				if ($architect) {
					echo '<div><b>Architect</b><br>' . $architect . '</div>';
				}
				if ($egineer) {
					echo '<div><b>Engineer</b><br>' . $egineer . '</div>';
				}
				echo '</section>';
			}
