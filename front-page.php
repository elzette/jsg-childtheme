	...
	*/
	...
	if ( have_posts() ) :
		...
		while ( have_posts() ) : the_post();
			...
			<li><figure class="featured-image" style="background-image: url( '<?php echo esc_url( $background[0] ); ?>' ); background-position: center;"></figure>
			<?php foreach ( (get_the_category() ) as $category ) {
					echo '<p class="project-info"><span class="service-name">' . esc_html( $category->cat_name ) . '</span> <em>' . get_field( 'project_area' ) . ' </em> <a href="' . get_the_permalink() . '" alt="' . get_the_title() . '" class="more-link">// View Project<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0" y="0" viewBox="0 0 7 12" xml:space="preserve"><path d="M0,0v3L3.5,6L0,9v3l7-6L0,0z"/></svg></a></p>';
}
			echo '</figure></li>';
		endwhile;
...
function jsg_add_services_section() {
	...
	if ( $rows ) {
		foreach ( $rows as $row ) {
			...
			echo '<a href="' . site_url() . '/services/' . $row['button_link'] . '"><section><figure style="background-image: url(' . esc_url( $image['sizes']['large'] ) . '); background-position: center;"></figure><h3><div class="button">' . $row['service_title'] . '</div></h3>';
			echo '</section></a>';
		}
	}
